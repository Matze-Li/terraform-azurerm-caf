resource "azuread_group" "group" {
  ...
  display_name            = var.azuread_groups.passthrough ? format("%s", var.azuread_groups.name) : format("%s%s", try(format("%s-", var.global_settings.prefixes.0), ""), var.azuread_groups.name)
